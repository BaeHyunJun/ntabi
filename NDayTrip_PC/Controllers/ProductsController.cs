...
namespace NDayTrip_PC.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public ActionResult Lists(string loc)
        {
            ...
            {
                defaultLists = defaultLists.Where(w => aaa.Contains(w.PDT_AREA_CODE)).ToList();
            }

            if (!string.IsNullOrEmpty(options))
            {
                defaultLists = defaultLists.Where(w => w.Option.Equals(options)).ToList();
            }
            ...
            var pdtData = DB.PDT_0.Where(w => (w.CORP_CODE + w.PDT_TYPE_CODE + w.PDT_IST_EMP_NO + w.PDT_YY + w.PDT_SEQ).Equals(code))
                                  .Join(DB.EMP_0,
                                       ...
                                       (a, b) => new
                                       {
                                           ...
                                           key = a.CORP_CODE + a.PDT_TYPE_CODE + a.PDT_IST_EMP_NO + a.PDT_YY + a.PDT_SEQ
                                       });



            var pdt_ImgLists = pdtData.Join(DB.PDT_2,
                                            a => a.key,
                                            b => b.CORP_CODE + b.PDT_TYPE_CODE + b.PDT_IST_EMP_NO + b.PDT_YY + b.PDT_SEQ,
                                            (a, b) => new
                                            {
        ...
        public JsonResult getDays(string code, string key, string trfCode = "", string prcCode = "")
        {
            ...
            //{
            //    var resultDays = results//.Where(w => w.state != "03" && !string.IsNullOrEmpty(w.state))
            //                            .ToList()
            //                            .GroupBy(g => new { g.day })
            //                            .Select(s => new
            //                            {
