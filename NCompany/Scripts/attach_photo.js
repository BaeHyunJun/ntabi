	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    	     welBtnConfirm.attr("src", "/Content/images/smartEditor/btn_confirm2.png");
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
    		 ...
    		 welBtnConfirm.attr("src", "/Content/images/smartEditor/btn_confirm.png");
    	 }
     }   
    ...
    function drop(ev) {
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type.toLowerCase())) {
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			...
			aFileList.push('	<a onclick="delImage(\'img' + nImageInfoCnt + '\')"><img class="del_button" src="/Content/Content/images/event/event/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
			aFileList.push('	</li> ');   
    ...
    function html5Upload() {	
    	...
    	sUploadURL = '/Editor/FileUploader_html5'; 	//upload URL
    ...
    function callAjaxForHTML5(tempFile, sUploadURL) {
    	...
    	var oAjax = jindo.$Ajax(sUploadURL, {
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
			    ...
			    console.log(sResString);
			    console.log("----------------------");
			    console.log("sResString.indexOf('NOWALLOW_')");
			    console.log(sResString.indexOf("NOWALLOW_"));
			    console.log("----------------------");
	...
	function onAjaxError (){
     ...
     function callFileUploader() {
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 	        ...
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/FileUploader',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
	...
	window.onload = function(){
	    ...
	    location.href.replace(/([^=|?]+)=([^&]*)(&|$)/g, function () {
	        oParameter[arguments[1]] = arguments[2];
	    });

	    if (oParameter.isFeature) {
	        bSupportDragAndDropAPI = false;
	    }
